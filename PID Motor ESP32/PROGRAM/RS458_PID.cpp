...
void speedControllerTick()
{
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        else if (fabsf(u) < PWM_MIN_RUN)
            u = copysignf(PWM_MIN_RUN, u);
...
void updateAndPrintRPM()
{
    ...
    lastPrint = now;
    if (targetRPM[0] != 0)
    { // Hanya print kalau bergerak supaya serial bersih
        Serial.print("M1:");
        Serial.print((int)rpmEMA[0]);
        Serial.print(" M2:");
        Serial.print((int)rpmEMA[1]);
        Serial.print(" M3:");
        Serial.print((int)rpmEMA[2]);
        Serial.print(" M4:");
        Serial.println((int)rpmEMA[3]);
    }
}
