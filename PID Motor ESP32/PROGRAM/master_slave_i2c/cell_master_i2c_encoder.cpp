...
// MASTER i2c untuk kirim ke slave 1,2,3
...
#define SLAVE_ADDRESS_2 11
#define SLAVE_ADDRESS_3 21
...
void loop()
{
    if (Serial.available() > 0)
    {
        ...
        if (angka < 5)
        {
            Wire.beginTransmission(SLAVE_ADDRESS_1); // kirim ke slave 1
            Wire.write((uint8_t)angka);              // kirim 1 byte
            err = Wire.endTransmission();
        }
        if (angka < 15 && angka > 9)
        {
            Wire.beginTransmission(SLAVE_ADDRESS_2); // kirim ke slave 2
            Wire.write((uint8_t)angka);              // kirim 1 byte
            err = Wire.endTransmission();
        }
        ...
        {
            // minta 1 byte respon
            uint8_t n = Wire.requestFrom(SLAVE_ADDRESS_1, (uint8_t)1);
    ...
    //     case '9': STOP_BRAKE(); break; // rem cepat
    //     case '0': STOP_COAST(); break; // rem lepas
    //     }
    // }

    updateAndPrintRPM(); // hitung & tampilkan RPM tiap 1 detik
    delay(10);           // kecilkan beban CPU
}
